...
@section('content')
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <div class="flex items-center space-x-3">
                    <div
                        class="w-12 h-12 bg-gradient-to-br from-green-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Navigate through sections easily - Each section saves independently
                        </p>
                    </div>
                </div>
            </div>
        ...
        @php
            ...
            // Determine the first available tab based on user permissions
            $defaultTab = null;
            if (auth()->user()->hasPermission('hero-sliders.manage')) {
                $defaultTab = 'hero_sliders';
            } else {
                // Find the first settings group the user has permission for
        ...
        <div class="flex flex-col lg:flex-row gap-6" x-data="{ 
            ...
            <div class="lg:hidden mb-4">
                <button @click="mobileMenuOpen = !mobileMenuOpen"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <div class="font-semibold text-gray-900 capitalize text-sm"
                                x-text="activeTab.replace('_', ' ')"></div>
                            <div class="text-xs text-gray-500">Settings Section</div>
                        </div>
                    </div>
                    <svg :class="mobileMenuOpen ? 'rotate-180' : ''" class="w-5 h-5 text-gray-500 transition-transform"
                ...
                <div x-show="mobileMenuOpen" x-transition @click.away="mobileMenuOpen = false"
                    ...
                    @if(auth()->user()->hasPermission('hero-sliders.manage'))
                        ...
                        <button @click="activeTab = 'hero_sliders'; mobileMenuOpen = false;"
                            ...
                            <div :class="activeTab === 'hero_sliders' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-600'"
                    ...
                    @foreach($settingsGroups as $group => $groupSettings)
                        @if(auth()->user()->hasPermission('homepage-' . $group . '.manage'))
                            ...
                            @endphp
                            <button @click="activeTab = '{{ $group }}'; mobileMenuOpen = false;"
                                ...
                                <div :class="activeTab === '{{ $group }}' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'"
                                    class="w-8 h-8 rounded-lg flex items-center justify-center">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-4">
                    ...
                    <nav class="p-2 space-y-1">
                        @if(auth()->user()->hasPermission('hero-sliders.manage'))
                            ...
                            <button @click="activeTab = 'hero_sliders'"
                                ...
                                <div :class="activeTab === 'hero_sliders' ? 'bg-purple-100' : 'bg-gray-100 group-hover:bg-purple-50'"
                                    class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors">
                                    <svg :class="activeTab === 'hero_sliders' ? 'text-purple-600' : 'text-gray-600 group-hover:text-purple-600'"
                        ...
                        @foreach($settingsGroups as $group => $groupSettings)
                            @if(auth()->user()->hasPermission('homepage-' . $group . '.manage'))
                                ...
                                <button @click="activeTab = '{{ $group }}'"
                                    ...
                                    class="w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all border-2 text-left group">
                                    <div :class="activeTab === '{{ $group }}' ? 'bg-green-100' : 'bg-gray-100 group-hover:bg-green-50'"
                                        class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors">
                                        <svg :class="activeTab === '{{ $group }}' ? 'text-green-600' : 'text-gray-600 group-hover:text-green-600'"
                                            class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $icon }}">
                                            </path>
                                        </svg>
            ...
            <div class="flex-1">
                @if(auth()->user()->hasPermission('hero-sliders.manage'))
                    ...
                    <div x-show="activeTab === 'hero_sliders'" x-transition:enter="transition ease-out duration-200"
                ...
                @foreach($settingsGroups as $group => $groupSettings)
                    @if(auth()->user()->hasPermission('homepage-' . $group . '.manage'))
                        <div x-show="activeTab === '{{ $group }}'" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 transform scale-95"
                            x-transition:enter-end="opacity-100 transform scale-100"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 transform scale-100"
