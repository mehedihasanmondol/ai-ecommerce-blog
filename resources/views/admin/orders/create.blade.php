...
@section('content')
    <div class="space-y-4" x-data="orderForm()" x-cloak
        @product-selected.window="addSelectedProduct($event.detail.productData || $event.detail)">
        ...
        <div x-show="showProductModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto pointer-events-none"
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="relative bg-white rounded-lg shadow-2xl max-w-md w-full p-6 pointer-events-auto border-2 border-gray-200"
                    @click.outside="closeProductModal()" @click.stop>
                    <!-- Close Button -->
                    <button @click="closeProductModal()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
                    ...
                    <template x-if="tempProduct">
                        <div>
                            ...
                            <div class="flex items-start space-x-3 mb-4 p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="flex-1">
                                    ...
                                    <template x-if="tempProduct.variant_name">
                                        <p class="text-xs text-gray-600" x-text="'Variant: ' + tempProduct.variant_name">
                                        </p>
                                    </template>
                                    ...
                                    <template x-if="stockRestrictionEnabled && tempProduct.stock_quantity !== undefined">
                                        <p class="text-xs mt-1"
                                            :class="tempProduct.stock_quantity > 0 ? 'text-green-600' : 'text-red-600'">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" x-model.number="tempProduct.quantity" min="1"
                                    :max="stockRestrictionEnabled ? (tempProduct.stock_quantity || 999) : 9999"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <p class="text-xs text-gray-500 mt-1" x-show="stockRestrictionEnabled"
                                    style="display: none;">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" x-model.number="tempProduct.price" step="0.01" min="0"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            ...
                            <div class="flex space-x-3">
                                ...
                                <button type="button" @click="confirmAddProduct()"
        ...
        <form id="order-form" action="{{ route('admin.orders.store') }}" method="POST"
            @submit="if(items.length === 0) { window.dispatchEvent(new CustomEvent('alert-toast', { detail: { type: 'error', message: 'Please add at least one product to the order' } })); $event.preventDefault(); return false; }">
            @csrf
                ...
                <div class="lg:col-span-2 space-y-4">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div class="p-4">
                                        ...
                                        <div class="flex items-start space-x-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-start justify-between">
                                                    <div class="flex-1">
                                                        <h4 class="text-sm font-semibold text-gray-900" x-text="item.name">
                                                        </h4>
                                                        <template x-if="item.variant_name">
                                                            <p class="text-xs text-gray-600 mt-0.5">Variant: <span
                                                                    x-text="item.variant_name"></span></p>
                                                        </template>
                                                        <p class="text-xs text-gray-500 mt-0.5">SKU: <span
                                                                x-text="item.sku"></span></p>
                                                        <template
                                                            x-if="stockRestrictionEnabled && item.stock_quantity !== undefined">
                                                            <p class="text-xs mt-1"
                                                                :class="item.stock_quantity > 0 ? 'text-green-600' : 'text-red-600'">
                                                ...
                                                <div class="grid grid-cols-3 gap-3 mt-3">
                                                    ...
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700 mb-1">Unit
                                                            Price (৳)</label>
                            ...
                            <div x-show="items.length === 0" class="text-center py-12 text-gray-500">
                                ...
                                <p class="text-xs text-gray-400 mt-1">Search and select products above to add them to the
                                    order</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Quick Select Customer
                                    (Optional)</label>
                                <select name="user_id" @change="fillCustomerData($event)"
                                    ...
                                    @foreach($users as $user)
                                        ...
                                        <option value="{{ $user->id }}" data-name="{{ $user->name }}"
                                            data-email="{{ $user->email }}" data-phone="{{ $user->mobile ?? '' }}"
                                            data-first-name="{{ $firstName }}" data-last-name="{{ $lastName }}"
                                            data-address="{{ $user->address ?? '' }}" data-city="{{ $user->city ?? 'Dhaka' }}"
                                ...
                                @if($users->isEmpty())
                                    <p class="text-xs text-gray-500 mt-1">No existing customers - Enter new customer details
                                        below</p>
                                @else
                            ...
                            <div>
                                ...
                                <input type="text" name="customer_name" id="customer_name" required
                                    value="{{ old('customer_name') }}" x-init="customer.name = '{{ old('customer_name') }}'"
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Phone <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="customer_phone" id="customer_phone" required
                                        value="{{ old('customer_phone') }}"
                                        x-init="customer.phone = '{{ old('customer_phone') }}'" x-model="customer.phone"
                                ...
                                <div>
                                    ...
                                    <input type="email" name="customer_email" id="customer_email" required
                                        ...
                                        x-init="customer.email = '{{ old('customer_email') }}'" x-model="customer.email"
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Address <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="customer_address" id="customer_address" required
                                    value="{{ old('customer_address') }}"
                                    x-init="customer.address = '{{ old('customer_address') }}'" x-model="customer.address"
                            ...
                            @if(auth()->user()->hasPermission('customers.update-info'))
                                <div x-show="selectedUserId && customerDataChanged" x-transition
                                    class="pt-3 border-t border-gray-200">
                                    <button type="button" @click="updateCustomerInfo()"
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <input type="number" name="shipping_cost" value="{{ old('shipping_cost', 60) }}"
                                    x-init="shipping = {{ old('shipping_cost', 60) }}" x-model.number="shipping" step="0.01"
                                    min="0" required
                            ...
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <input type="number" name="discount_amount" value="{{ old('discount_amount', 0) }}"
                                    x-init="discount = {{ old('discount_amount', 0) }}" x-model.number="discount"
                                    step="0.01" min="0"
                                    class="w-24 text-right text-sm px-2 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
                            </div>
                            ...
                            <div class="border-t border-gray-200 pt-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-2xl font-bold text-blue-600"
                                        x-text="'৳' + calculateTotal().toFixed(2)">৳0.00</span>
                                </div>
                            </div>

                            <!-- Payment Method -->
                            <div class="pt-3 border-t border-gray-200">
                                <label class="block text-xs font-medium text-gray-700 mb-2">Payment Method <span
                                        class="text-red-500">*</span></label>
                                <select name="payment_method" required
                                    class="w-full text-sm px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                    <option value="cod" {{ old('payment_method') == 'cod' ? 'selected' : '' }}>Cash on
                                        Delivery</option>
                            ...
                            <!-- Payment Status -->
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-2">Payment Status <span
                                        class="text-red-500">*</span></label>
