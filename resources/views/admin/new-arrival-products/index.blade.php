...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        @if(auth()->user()->hasPermission('new-arrivals.edit'))
            <x-admin.section-settings :sectionEnabled="$sectionEnabled" :sectionTitle="$sectionTitle"
                ...
                updateTitleRoute="{{ route('admin.new-arrival-products.update-title') }}" sectionName="New Arrivals" />
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="{{ auth()->user()->hasPermission('new-arrivals.create') ? 'lg:col-span-2' : 'lg:col-span-3' }}">
                <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            @foreach($newArrivalProducts as $newArrival)
                                <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition cursor-move"
                                    ...
                                    <div class="flex items-center space-x-4 flex-1">
                                        ...
                                        @if($newArrival->product)
                                            ...
                                            @if($newArrival->product->getPrimaryThumbnailUrl())
                                                <img src="{{ $newArrival->product->getPrimaryThumbnailUrl() }}"
                                                    alt="{{ $newArrival->product->name }}" class="w-16 h-16 object-cover rounded-md">
                                            @else
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @if(auth()->user()->hasPermission('new-arrivals.edit'))
                                            <!-- Toggle Status -->
                                            <button onclick="toggleStatus({{ $newArrival->id }})"
                                        ...
                                        @if(auth()->user()->hasPermission('new-arrivals.delete'))
                                            <!-- Delete -->
                                            <form action="{{ route('admin.new-arrival-products.destroy', $newArrival) }}" method="POST"
                                                class="inline" id="delete-form-{{ $newArrival->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                        ...
                                        @if(!auth()->user()->hasPermission('new-arrivals.edit') && !auth()->user()->hasPermission('new-arrivals.delete'))
    ...
    @push('scripts')
        ...
        <script>
            // Sortable List
            const list = document.getElementById('newarrival-list');
            if (list) {
                new Sortable(list, {
                    animation: 150,
                    handle: '.cursor-move',
