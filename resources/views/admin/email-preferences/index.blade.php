...
@section('content')
    <div class="container-fluid px-4 py-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-xs text-gray-500">
                            {{ round(($stats['order_updates_enabled'] / max($stats['total_customers'], 1)) * 100) }}%
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-xs text-gray-500">
                            {{ round(($stats['promotions_enabled'] / max($stats['total_customers'], 1)) * 100) }}%
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-xs text-gray-500">
                            {{ round(($stats['newsletter_enabled'] / max($stats['total_customers'], 1)) * 100) }}%
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['recommendations_enabled']) }}
                        </p>
                        <p class="text-xs text-gray-500">
                            {{ round(($stats['recommendations_enabled'] / max($stats['total_customers'], 1)) * 100) }}%
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow mb-6">
            <div class="p-5">
                <form method="GET" action="{{ route('admin.email-preferences.index') }}"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
                        ...
                        @forelse($users as $user)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if(auth()->user()->hasPermission('email-preferences.edit'))
                                        <button
                                            onclick="togglePreference({{ $user->id }}, 'order_updates', {{ $user->email_order_updates ? 'true' : 'false' }})"
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->email_order_updates ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                            <i class="fas fa-{{ $user->email_order_updates ? 'check' : 'times' }} mr-1"></i>
                                            {{ $user->email_order_updates ? 'ON' : 'OFF' }}
                                        </span>
                                    @endif
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if(auth()->user()->hasPermission('email-preferences.edit'))
                                        <button
                                            onclick="togglePreference({{ $user->id }}, 'promotions', {{ $user->email_promotions ? 'true' : 'false' }})"
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->email_promotions ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                            <i class="fas fa-{{ $user->email_promotions ? 'check' : 'times' }} mr-1"></i>
                                            {{ $user->email_promotions ? 'ON' : 'OFF' }}
                                        </span>
                                    @endif
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if(auth()->user()->hasPermission('email-preferences.edit'))
                                        <button
                                            onclick="togglePreference({{ $user->id }}, 'newsletter', {{ $user->email_newsletter ? 'true' : 'false' }})"
                                    ...
                                    @else
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->email_newsletter ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                    <i class="fas fa-{{ $user->email_newsletter ? 'check' : 'times' }} mr-1"></i>
                                                    {{ $user->email_newsletter ? 'ON' : 'OFF' }}
                                                </span>
                                            @endif
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            @if(auth()->user()->hasPermission('email-preferences.edit'))
                                                <button
                                                    onclick="togglePreference({{ $user->id }}, 'recommendations', {{ $user->email_recommendations ? 'true' : 'false' }})"
                                            ...
                                            @else
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->email_recommendations ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                    <i class="fas fa-{{ $user->email_recommendations ? 'check' : 'times' }} mr-1"></i>
                                                    {{ $user->email_recommendations ? 'ON' : 'OFF' }}
                                                </span>
                                            @endif
            ...
            <div id="bulk-actions"
                class="hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white rounded-lg shadow-lg border border-gray-200 px-6 py-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button onclick="bulkUpdate(true)"
        ...
        @push('scripts')
            <script>
                ...
                async function togglePreference(userId, preference, currentValue) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            const buttons = document.querySelectorAll(`button[onclick*="togglePreference(${userId}, '${preference}'"]`);
                            buttons.forEach(button => {
                                // Update button appearance
                                if (newValue) {
                                    button.className = 'preference-toggle inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition-colors bg-green-100 text-green-800 hover:bg-green-200';
                                    button.innerHTML = '<i class="fas fa-check mr-1"></i> ON';
                                } else {
                                    button.className = 'preference-toggle inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition-colors bg-gray-100 text-gray-800 hover:bg-gray-200';
                                    button.innerHTML = '<i class="fas fa-times mr-1"></i> OFF';
                                }
                                // Update onclick attribute
                                button.setAttribute('onclick', `togglePreference(${userId}, '${preference}', ${newValue})`);
                            });
                            ...
                            window.dispatchEvent(new CustomEvent('show-toast', {
                                detail: { message: data.message || 'Failed to update preference', type: 'error' }
                            }));
                        }
                    } catch (error) {
                        console.error('Error:', error);
                        window.dispatchEvent(new CustomEvent('show-toast', {
                            detail: { message: 'An error occurred while updating preference', type: 'error' }
                        }));
                    }
                }
                ...
                async function bulkUpdate(enabled) {
                    ...
                    if (!preference) {
                        window.dispatchEvent(new CustomEvent('show-toast', {
                            detail: { message: 'Please select a preference type', type: 'warning' }
                        }));
                        return;
                    }
                    ...
                    if (selectedIds.length === 0) {
                        window.dispatchEvent(new CustomEvent('show-toast', {
                            detail: { message: 'Please select at least one customer', type: 'warning' }
                        }));
                        return;
                    }
                    ...
                    window.dispatchEvent(new CustomEvent('confirm-modal', {
                        detail: {
                            title: `${enabled ? 'Enable' : 'Disable'} Preference`,
                            message: `Are you sure you want to ${enabled ? 'enable' : 'disable'} ${preference.replace('_', ' ')} for ${selectedIds.length} customer(s)?`,
                            onConfirm: () => performBulkUpdate(selectedCheckboxes, selectedIds, preference, enabled)
                        }
                    }));
                }

                // Perform the actual bulk update
                async function performBulkUpdate(selectedCheckboxes, selectedIds, preference, enabled) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            // Update UI for all affected rows
                            selectedCheckboxes.forEach(checkbox => {
                                ...
                                buttons.forEach(button => {
                                    ...
                                    button.setAttribute('onclick', `togglePreference(${userId}, '${preference}', ${enabled})`);
                                });
                            ...
                            // Reset preference dropdown
                            document.getElementById('bulk-preference').value = '';
                            ...
                            window.dispatchEvent(new CustomEvent('show-toast', {
                                detail: { message: data.message || 'Failed to update preferences', type: 'error' }
                            }));
                        ...
                        console.error('Error:', error);
                        window.dispatchEvent(new CustomEvent('show-toast', {
                            detail: { message: 'An error occurred while updating preferences', type: 'error' }
                        }));
