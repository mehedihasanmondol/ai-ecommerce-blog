                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        <li class="inline-flex items-center">
                            <a href="{{ route('admin.product-questions.index') }}"
                                class="text-blue-600 hover:text-blue-800">Questions</a>
                        </li>
                        <li>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                        ...
                        @if($question->status == 'approved')
                            <span
                                class="px-3 py-1 text-sm font-semibold rounded-full bg-green-100 text-green-800">Approved</span>
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                <p class="text-sm text-gray-600 mb-1"><strong>Asked by:</strong>
                                    {{ $question->author_name }}</p>
                                @if($question->user)
                            ...
                            <div>
                                <p class="text-sm text-gray-600 mb-1"><strong>Date:</strong>
                                    {{ $question->created_at->format('M d, Y h:i A') }}</p>
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            <span class="text-sm text-gray-600">
                                <i class="fas fa-thumbs-down text-red-600"></i> {{ $question->not_helpful_count }} Not
                                Helpful
                            </span>
                        </div>
                        ...
                        @if($question->status == 'pending')
                            <div class="flex gap-2">
                                @if(auth()->user()->hasPermission('questions.approve'))
                                    <form action="{{ route('admin.questions.approve', $question->id) }}" method="POST"
                                        id="approve-question-{{ $question->id }}">
                                        @csrf
                                        <button type="button" onclick="window.dispatchEvent(new CustomEvent('confirm-modal', { 
                                                    detail: { 
                                ...
                                @endif
                                @if(auth()->user()->hasPermission('questions.reject'))
                                    <form action="{{ route('admin.questions.reject', $question->id) }}" method="POST"
                                        id="reject-question-{{ $question->id }}">
                                        @csrf
                                        <button type="button" onclick="window.dispatchEvent(new CustomEvent('confirm-modal', { 
                                                    detail: { 
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="p-6">
                        @forelse($question->answers as $answer)
                            <div
                                class="border rounded-lg p-4 mb-4 {{ $answer->is_best_answer ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                                ...
                                <div class="flex gap-2">
                                    @if($answer->status == 'pending')
                                        @if(auth()->user()->hasPermission('answers.approve'))
                                            <form action="{{ route('admin.answers.approve', $answer->id) }}" method="POST"
                                                id="approve-answer-{{ $answer->id }}">
                                                @csrf
                                                <button type="button" onclick="window.dispatchEvent(new CustomEvent('confirm-modal', { 
                                                                    detail: { 
                                        ...
                                        @endif
                                        @if(auth()->user()->hasPermission('answers.reject'))
                                            <form action="{{ route('admin.answers.reject', $answer->id) }}" method="POST"
                                                id="reject-answer-{{ $answer->id }}">
                                                @csrf
                                                <button type="button" onclick="window.dispatchEvent(new CustomEvent('confirm-modal', { 
                                                                    detail: { 
                                                                        title: 'Reject Answer', 
                                        ...
                                        @endif
                                    @endif
                                    @if(!$answer->is_best_answer && $answer->status == 'approved' && auth()->user()->hasPermission('answers.best'))
                                        <form action="{{ route('admin.answers.best', $answer->id) }}" method="POST"
                                            id="best-answer-{{ $answer->id }}">
                                            @csrf
                                            <button type="button" onclick="window.dispatchEvent(new CustomEvent('confirm-modal', { 
                                                                detail: { 
            ...
            <div class="lg:col-span-1">
                    ...
                    <div class="p-6">
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between">
                                ...
                                <strong
                                    class="text-gray-900">{{ $question->answers->where('status', 'approved')->count() }}</strong>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Pending Answers</span>
                                <strong
                                    class="text-gray-900">{{ $question->answers->where('status', 'pending')->count() }}</strong>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="p-6 space-y-2">
                        <a href="{{ route('products.show', $question->product->slug) }}#qa" target="_blank"
                        ...
                        @if(auth()->user()->hasPermission('questions.delete'))
                            <form action="{{ route('admin.product-questions.destroy', $question->id) }}" method="POST"
