...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    @php
    // Find the first tab that user has permission for
    $firstPermittedTab = null;
    foreach($settings as $group => $groupSettings) {
    $permission = "site-settings-{$group}.manage";
    if (auth()->user()->hasPermission($permission)) {
    $firstPermittedTab = $group;
    break;
    }
    }
    // Fallback to first tab if somehow no permissions (shouldn't happen due to controller check)
    $initialTab = $firstPermittedTab ?? array_key_first($settings->toArray());
    @endphp

    <div class="flex flex-col lg:flex-row gap-6"
        x-data="{ activeTab: '{{ $initialTab }}', mobileMenuOpen: false }">
        ...
        <div class="lg:hidden mb-4">
            ...
            <div x-show="mobileMenuOpen" x-transition @click.away="mobileMenuOpen = false"
                ...
                'typography' => 'M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129',
                ];
                ...
                <button @click="activeTab = '{{ $group }}'; mobileMenuOpen = false;"
                    ...
                    <div :class="activeTab === '{{ $group }}' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-4">
                ...
                <nav class="p-2 space-y-1">
                    ...
                    <button @click="activeTab = '{{ $group }}'"
                        ...
                        <div :class="activeTab === '{{ $group }}' ? 'bg-blue-100' : 'bg-gray-100 group-hover:bg-blue-50'"
                            class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors">
                            <svg :class="activeTab === '{{ $group }}' ? 'text-blue-600' : 'text-gray-600 group-hover:text-blue-600'"
                        ...
                        <svg :class="activeTab === '{{ $group }}' ? 'opacity-100' : 'opacity-0'"
                            class="w-5 h-5 transition-opacity" fill="currentColor" viewBox="0 0 20 20">
