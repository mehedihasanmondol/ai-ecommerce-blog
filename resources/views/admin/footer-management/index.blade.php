    ...
    <div class="bg-white rounded-lg shadow-md">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                @if(auth()->user()->hasPermission('footer-general.manage'))
                ...
                @if(auth()->user()->hasPermission('footer-links.manage'))
                ...
                @if(auth()->user()->hasPermission('footer-blog.manage'))
                ...
                @if(auth()->user()->hasPermission('footer-social.manage'))
                ...
                @if(auth()->user()->hasPermission('footer-mobile.manage'))
                ...
                @if(auth()->user()->hasPermission('footer-rewards.manage'))
                ...
                @if(auth()->user()->hasPermission('footer-newspaper.manage'))
                <button onclick="showTab('newspaper')" id="tab-newspaper" class="tab-button px-6 py-4 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                    <i class="fas fa-newspaper mr-2"></i>Newspaper Footer
                </button>
                @endif
            </nav>
        </div>
        ...
        <div class="p-6">
            ...
            <div id="content-settings" class="tab-content">
                ...
                @if(auth()->user()->hasPermission('footer-general.manage') && auth()->user()->hasPermission('footer.edit'))
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                    <p class="text-yellow-800 text-sm"><i class="fas fa-lock mr-2"></i>You have view-only access to general settings.</p>
                </div>
                <div class="space-y-6">
            ...
            <div id="content-links" class="tab-content hidden">
                ...
                @if(auth()->user()->hasPermission('footer-links.manage') && auth()->user()->hasPermission('footer.edit'))
                <x-admin.footer-section-toggle
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <p class="text-yellow-800 text-sm"><i class="fas fa-lock mr-2"></i>You have view-only access. Footer links can only be edited with footer-links.manage permission.</p>
                </div>
                @endif
            </div>
            @endif
            ...
            <div id="content-blog" class="tab-content hidden">
                ...
                @if(auth()->user()->hasPermission('footer-blog.manage') && auth()->user()->hasPermission('footer.edit'))
                <x-admin.footer-section-toggle
            ...
            <div id="content-social" class="tab-content hidden">
                ...
                @if(auth()->user()->hasPermission('footer-social.manage') && auth()->user()->hasPermission('footer.edit'))
                <x-admin.footer-section-toggle
            ...
            <div id="content-mobile" class="tab-content hidden">
                @if(auth()->user()->hasPermission('footer-mobile.manage') && auth()->user()->hasPermission('footer.edit'))
            ...
            <div id="content-rewards" class="tab-content hidden">
                ...
                @if(auth()->user()->hasPermission('footer-rewards.manage') && auth()->user()->hasPermission('footer.edit'))
                <form action="{{ route('admin.footer-management.update-settings') }}" method="POST">
                    @csrf
                    <div class="space-y-6">
            ...
            <!-- Newspaper Footer Tab -->
            @if(auth()->user()->hasPermission('footer-newspaper.manage'))
            <div id="content-newspaper" class="tab-content hidden">
                @if(auth()->user()->hasPermission('footer-newspaper.manage') && auth()->user()->hasPermission('footer.edit'))
                <form action="{{ route('admin.footer-management.update-settings') }}" method="POST">
                    @csrf
                    <div class="space-y-6">
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                            <p class="text-blue-800 text-sm"><i class="fas fa-info-circle mr-2"></i>Configure the content for the second and third columns in the newspaper footer. The first column displays the site logo automatically.</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Second Column Content</label>
                            <textarea name="newspaper_second_column" id="newspaper-second-column-editor" class="ckeditor-content-minimal">{{ $settings['newspaper']->firstWhere('key', 'newspaper_second_column')->value ?? '' }}</textarea>
                            <p class="text-xs text-gray-500 mt-1">Content for the second column in newspaper footer (supports HTML)</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Third Column Content</label>
                            <textarea name="newspaper_third_column" id="newspaper-third-column-editor" class="ckeditor-content-minimal">{{ $settings['newspaper']->firstWhere('key', 'newspaper_third_column')->value ?? '' }}</textarea>
                            <p class="text-xs text-gray-500 mt-1">Content for the third column in newspaper footer (supports HTML)</p>
                        </div>

                        <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition">
                            <i class="fas fa-save mr-2"></i>Save Newspaper Footer Settings
                        </button>
                    </div>
                </form>
                @else
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                    <p class="text-yellow-800 text-sm"><i class="fas fa-lock mr-2"></i>You have view-only access to newspaper footer settings.</p>
                </div>
                <div class="space-y-6">
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Second Column Content</label>
                        <div class="text-gray-900 prose max-w-none">{!! $settings['newspaper']->firstWhere('key', 'newspaper_second_column')->value ?? 'Not set' !!}</div>
                    </div>
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Third Column Content</label>
                        <div class="text-gray-900 prose max-w-none">{!! $settings['newspaper']->firstWhere('key', 'newspaper_third_column')->value ?? 'Not set' !!}</div>
                    </div>
                </div>
                @endif
            </div>
...
<script>
    ...
    // Auto-open first available tab on page load
