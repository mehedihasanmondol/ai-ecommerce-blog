    ...
    <div class="container mx-auto px-4 py-8">
        ...
        @if(auth()->user()->hasPermission('best-sellers.edit'))
            <x-admin.section-settings :sectionEnabled="$sectionEnabled" :sectionTitle="$sectionTitle"
                ...
                updateTitleRoute="{{ route('admin.best-seller-products.update-title') }}" sectionName="Best Seller Products" />
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="{{ auth()->user()->hasPermission('best-sellers.create') ? 'lg:col-span-2' : 'lg:col-span-3' }}">
                <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            @foreach($bestSellerProducts as $bestSeller)
                                <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition cursor-move"
                                    ...
                                    <div class="flex items-center space-x-4 flex-1">
                                        ...
                                        @if($bestSeller->product)
                                            ...
                                            @if($bestSeller->product->getPrimaryThumbnailUrl())
                                                <img src="{{ $bestSeller->product->getPrimaryThumbnailUrl() }}"
                                                    alt="{{ $bestSeller->product->name }}" class="w-16 h-16 object-cover rounded-md">
                                            @else
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @if(auth()->user()->hasPermission('best-sellers.edit'))
                                            <!-- Toggle Status -->
                                            <button onclick="toggleStatus({{ $bestSeller->id }})"
                                        ...
                                        @if(auth()->user()->hasPermission('best-sellers.delete'))
                                            <!-- Delete -->
                                            <form action="{{ route('admin.best-seller-products.destroy', $bestSeller) }}" method="POST"
                                                class="inline" id="delete-form-{{ $bestSeller->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                        ...
                                        @if(!auth()->user()->hasPermission('best-sellers.edit') && !auth()->user()->hasPermission('best-sellers.delete'))
