...
@section('content')
    <div class="container-fluid px-4 py-6">
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <form id="post-form" action="{{ route('admin.blog.posts.update', $post->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-white rounded-lg shadow">
                            ...
                            <div class="px-6 py-3 border-t border-gray-100">
                                <div class="flex items-center text-sm">
                                    ...
                                    <input type="text" name="slug" id="post-slug" value="{{ old('slug', $post->slug) }}"
                    ...
                    <div class="space-y-6">
                        ...
                        {{-- Dynamic Tick Marks - Expandable --}}
                        @if(auth()->user()->hasPermission('posts.tick-marks'))
                            <div class="bg-white rounded-lg shadow"
                                x-data="{ expanded: {{ $post->tickMarks->count() > 0 ? 'true' : 'false' }} }">
                                <!-- Expandable Header -->
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex items-center gap-1" x-show="!expanded">
                                            @foreach($post->tickMarks as $tickMark)
                                                <span class="inline-flex items-center p-1 rounded"
                                                    style="background-color: {{ $tickMark->bg_color }}; color: {{ $tickMark->text_color }};"
                                ...
                                <input type="hidden" name="is_verified"
                                    value="{{ old('is_verified', $post->is_verified) ? '1' : '0' }}">
                                <input type="hidden" name="is_editor_choice"
                                    value="{{ old('is_editor_choice', $post->is_editor_choice) ? '1' : '0' }}">
                                <input type="hidden" name="is_trending"
                                    value="{{ old('is_trending', $post->is_trending) ? '1' : '0' }}">
                                <input type="hidden" name="is_premium"
                                    value="{{ old('is_premium', $post->is_premium) ? '1' : '0' }}">
                            </div>
                        ...
                        <div class="bg-white rounded-lg shadow p-6">
                            ...
                            <div id="youtube-preview"
                                class="mb-4 {{ $post->youtube_url && $post->youtube_video_id ? '' : 'hidden' }}">
                                <p class="text-sm font-medium text-gray-700 mb-2">Video Preview:</p>
                                <div class="relative" style="padding-bottom: 56.25%; height: 0;">
                                    <iframe id="youtube-iframe" src="{{ $post->youtube_embed_url ?? '' }}" frameborder="0"
                            ...
                            <div class="space-y-1 max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-3"
                                ...
                                @forelse($categories as $category)
                                    <label class="flex items-start hover:bg-gray-50 p-2 rounded cursor-pointer group">
                                        <input type="checkbox" name="categories[]" value="{{ $category->id }}" {{ in_array($category->id, old('categories', $post->categories->pluck('id')->toArray())) ? 'checked' : '' }}
                        ...
                        <div class="bg-white rounded-lg shadow p-6" x-data="{ 
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                    ...
                                    @if($post->products->count() > 0)
                                        <span
                                            class="ml-2 px-2 py-0.5 bg-green-100 text-green-800 text-xs font-semibold rounded-full">
                            ...
                            <div x-show="showProducts" x-collapse
                                ...
                                id="product-list-container-edit">
                                @php
                                    $attachedProductIds = old('products', $post->products->pluck('id')->toArray());
                                @endphp
                                @forelse($products as $product)
                                    <label class="flex items-center hover:bg-gray-50 p-2 rounded cursor-pointer">
                                        <input type="checkbox" name="products[]" value="{{ $product->id }}" {{ in_array($product->id, $attachedProductIds) ? 'checked' : '' }}
                                            class="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500">
    ...
    @push('scripts')
        <script>
            // Pass permission st    atus to JavaScript
