    ...
    <div class="container mx-auto px-4 py-8">
        ...
        @if(auth()->user()->hasPermission('sale-offers.edit'))
            <x-admin.section-settings :sectionEnabled="$sectionEnabled" :sectionTitle="$sectionTitle"
                ...
                updateTitleRoute="{{ route('admin.sale-offers.update-title') }}" sectionName="Sale Offers" />
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Add Product Search -->
            @if(auth()->user()->hasPermission('sale-offers.create'))
            ...
            <!-- Sale Offers List -->
            <div class="{{ auth()->user()->hasPermission('sale-offers.create') ? 'lg:col-span-2' : 'lg:col-span-3' }}">
                <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            @foreach($saleOffers as $offer)
                                <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition cursor-move"
                                    ...
                                    <div class="flex items-center space-x-4 flex-1">
                                        ...
                                        @if($offer->product)
                                            ...
                                            @if($offer->product->getPrimaryThumbnailUrl())
                                                <img src="{{ $offer->product->getPrimaryThumbnailUrl() }}" alt="{{ $offer->product->name }}"
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if(auth()->user()->hasPermission('sale-offers.delete'))
                                            <!-- Delete -->
                                            <form action="{{ route('admin.sale-offers.destroy', $offer) }}" method="POST" class="inline"
                                        ...
                                        @if(!auth()->user()->hasPermission('sale-offers.edit') && !auth()->user()->hasPermission('sale-offers.delete'))
