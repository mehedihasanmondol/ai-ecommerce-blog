            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th
                            class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider relative">
                            <span class="{{ count($selectedPosts) > 0 ? 'invisible' : '' }}">Actions</span>
                            @if(count($selectedPosts) > 0 && auth()->user()->hasPermission('posts.delete'))
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="{{ $post->featured_image ? 'ml-4' : '' }}">
                                        <div class="text-sm font-medium text-gray-900">{{ Str::limit($post->title, 50) }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex justify-center">
                                    ...
                                    @elseif($post->author->media)
                                        <img src="{{ $post->author->media->small_url }}" alt="{{ $post->author->name }}"
                                            title="{{ $post->author->name }}" class="h-8 w-8 rounded-full object-cover">
                                ...
                                @if(auth()->user()->hasPermission('posts.edit'))
                                    <div class="mt-2">
                                        <button wire:click="toggleFeatured({{ $post->id }})"
                                            ...
                                            <svg class="w-5 h-5" fill="{{ $post->is_featured ? 'currentColor' : 'none' }}"
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: @entangle('showDeleteModal') }">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative rounded-lg shadow-xl max-w-md w-full p-6 border border-gray-200"
                    ...
                    <p class="text-sm text-gray-500 text-center mb-6">Are you sure you want to delete this post? This action
                        cannot be undone.</p>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: @entangle('showBulkDeleteModal') }">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative rounded-lg shadow-xl max-w-md w-full p-6 border border-gray-200"
                    ...
                    <h3 class="text-lg font-medium text-gray-900 text-center mb-2">Delete {{ count($selectedPosts) }}
                        Post(s)</h3>
                    <p class="text-sm text-gray-500 text-center mb-6">Are you sure you want to delete
                        {{ count($selectedPosts) }} selected post(s)? This action cannot be undone.</p>
