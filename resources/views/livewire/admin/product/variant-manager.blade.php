        ...
        <div class="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-6">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    ...
                    <p class="mt-1 text-sm text-yellow-700">You do not have permission to manage product variants. Contact
                        your administrator to request the <strong>products.variants</strong> permission.</p>
                </div>
            </div>
        </div>
    @endif
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" wire:model="generatedVariants.{{ $index }}.name"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg" readonly>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">SKU</label>
                                    <input type="text" wire:model="generatedVariants.{{ $index }}.sku"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg" placeholder="Auto">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Price *</label>
                                    <input type="number" wire:model="generatedVariants.{{ $index }}.price" step="0.01"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg" placeholder="0.00">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Stock</label>
                                    <input type="number" wire:model="generatedVariants.{{ $index }}.stock_quantity"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg" placeholder="0">
                                </div>
                            </div>
                        </div>
                    @endforeach
