                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">
                            {{ \App\Models\ContactMessage::where('status', 'replied')->count() }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-600">
                            {{ \App\Models\ContactMessage::where('status', 'archived')->count() }}</p>
                    </div>
                    ...
                    @forelse($messages as $message)
                        <tr class="hover:bg-gray-50 {{ $message->status == 'unread' ? 'bg-blue-50' : '' }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div
                                        class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white font-semibold">
                                        {{ strtoupper(substr($message->name, 0, 1)) }}
                                    </div>
                                    <div class="ml-4">
                                        <div
                                            class="text-sm font-medium text-gray-900 {{ $message->status == 'unread' ? 'font-bold' : '' }}">
                                            {{ $message->name }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                @else
                                    <span
                                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-200 text-gray-600">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if($message->status == 'unread' && auth()->user()->hasPermission('contact-messages.update-status'))
                                        <button wire:click="markAsRead({{ $message->id }})"
                                            class="text-gray-600 hover:text-gray-900" title="Mark as Read">
                                            <i class="fas fa-envelope-open"></i>
                                        </button>
                                    @endif

                                    @if($message->status != 'replied' && auth()->user()->hasPermission('contact-messages.update-status'))
                                        <button wire:click="markAsReplied({{ $message->id }})"
                                            class="text-green-600 hover:text-green-900" title="Mark as Replied">
                                            <i class="fas fa-check"></i>
                                        </button>
                                    @endif

                                    @if($message->status != 'archived' && auth()->user()->hasPermission('contact-messages.update-status'))
                                        <button wire:click="archive({{ $message->id }})"
                                            class="text-yellow-600 hover:text-yellow-900" title="Archive">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-blue-600"><a
                                            href="mailto:{{ $viewingMessage->email }}">{{ $viewingMessage->email }}</a></p>
                                </div>
                                @if($viewingMessage->phone)
                                    <div>
                                        <span class="text-sm font-semibold text-gray-600">Phone:</span>
                                        <p class="text-blue-600"><a
                                                href="tel:{{ $viewingMessage->phone }}">{{ $viewingMessage->phone }}</a></p>
                                    </div>
                                @endif
                                <div>
                                ...
                                <div>
                                    ...
                                    <p>
                                        @if($viewingMessage->status == 'unread')
                                            <span
                                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">Unread</span>
                                        @elseif($viewingMessage->status == 'read')
                                            <span
                                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">Read</span>
                                        @elseif($viewingMessage->status == 'replied')
                                            <span
                                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Replied</span>
                                        @else
                                            <span
                                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-200 text-gray-600">Archived</span>
                                        @endif
                                    </p>
                                </div>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: @entangle('deletingMessageId').live }">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative rounded-lg shadow-xl max-w-md w-full p-6 border border-gray-200"
                    ...
                    <p class="text-sm text-gray-500 text-center mb-6">Are you sure you want to delete this message? This
