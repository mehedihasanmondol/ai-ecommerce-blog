<div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <!-- Success Message -->
        @if($successMessage)
            <div class="mb-6 bg-green-50 border border-green-200 rounded-lg p-4 flex items-start" x-data="{ show: true }"
        ...
        @if(!auth()->user()->hasPermission('system.maintenance'))
            <div class="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start">
                ...
                <div class="flex-1">
                    <p class="text-yellow-800 font-medium">Restricted Access</p>
                    <p class="text-yellow-700 text-sm mt-1">You do not have permission to manage maintenance mode. This
                        section is view-only. Contact your administrator to request maintenance management permissions.</p>
                </div>
            </div>
        @endif
        ...
        <div
            class="mb-8 {{ $maintenanceMode ? 'bg-yellow-50 border-yellow-300' : 'bg-gray-50 border-gray-200' }} border-2 rounded-xl p-6 transition-colors">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <h3
                        class="text-lg font-semibold {{ $maintenanceMode ? 'text-yellow-900' : 'text-gray-900' }} flex items-center">
                        @if($maintenanceMode)
                ...
                <!-- Toggle Switch -->
                <div class="ml-4">
                    <button wire:click="toggleMaintenanceMode" wire:loading.attr="disabled" type="button"
                        {{ !auth()->user()->hasPermission('system.maintenance') ? 'disabled' : '' }}
                        ...
                        <span
                            ...
                            <span wire:loading wire:target="toggleMaintenanceMode"
        ...
        <form wire:submit="updateSettings">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="maintenanceTitle" wire:model="maintenanceTitle"
                        {{ !auth()->user()->hasPermission('system.maintenance') ? 'disabled' : '' }}
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed @error('maintenanceTitle') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <textarea id="maintenanceMessage" wire:model="maintenanceMessage" rows="4"
                        {{ !auth()->user()->hasPermission('system.maintenance') ? 'disabled' : '' }}
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed @error('maintenanceMessage') border-red-500 @enderror"
                        placeholder="Sorry for the inconvenience. We're performing some maintenance..."></textarea>
                ...
                <div>
                    ...
                    @if($currentImage)
                        <div class="mb-4 relative inline-block">
                            <img src="{{ asset('storage/' . $currentImage) }}" alt="Maintenance image"
                    ...
                    <div class="mt-2">
                        <input type="file" wire:model="maintenanceImage" accept="image/*"
                            {{ !auth()->user()->hasPermission('system.maintenance') ? 'disabled' : '' }}
                            class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50">
