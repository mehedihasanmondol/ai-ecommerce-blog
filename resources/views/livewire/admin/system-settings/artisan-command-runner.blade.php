...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    ...
    @if(!auth()->user()->hasPermission('system.artisan'))
        <div class="mx-6 mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div class="flex items-start">
                ...
                <div class="ml-3 flex-1">
                    <h3 class="text-sm font-medium text-yellow-800">Permission Restricted</h3>
                    <p class="mt-2 text-sm text-yellow-700">You do not have permission to execute artisan commands. The command selection and execution are disabled. Contact your administrator to request artisan command execution permissions.</p>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="p-6 space-y-6">
        ...
        <div>
            ...
            <select wire:model.live="selectedCommand" {{ !auth()->user()->hasPermission('system.artisan') ? 'disabled' : '' }}
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all disabled:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <div x-data="{ showWarning: true }" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" wire:model="customCommand" placeholder="e.g., cache:clear --tags=views"
                            {{ !auth()->user()->hasPermission('system.artisan') ? 'disabled' : '' }}
                            class="w-full pl-32 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent font-mono text-sm disabled:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">
                    </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-gray-200">
            ...
            <div class="flex space-x-3">
                ...
                <button wire:click="executeCommand" wire:loading.attr="disabled" wire:target="executeCommand"
                    wire:loading.class="opacity-50 cursor-not-allowed" wire:target="executeCommand"
                    {{ !auth()->user()->hasPermission('system.artisan') ? 'disabled' : '' }}
                    class="px-6 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all shadow-md hover:shadow-lg flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
                    <svg wire:loading.remove wire:target="executeCommand" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                    </svg>
                    <svg wire:loading wire:target="executeCommand" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                    ...
                    <span wire:loading.remove wire:target="executeCommand">Execute Command</span>
    ...
    @if($commandOutput)
        <div class="p-6 bg-gray-50 border-t border-gray-200">
            <div class="flex items-center justify-between mb-3">
                ...
                <button onclick="navigator.clipboard.writeText(this.closest('.p-6').querySelector('pre').textContent)"
    ...
    <div class="p-4 bg-indigo-50 border-t border-indigo-100 rounded-b-xl">
        <div class="flex items-start space-x-2 text-xs text-indigo-900">
            ...
            <div>
                ...
                <p class="mt-1">Only whitelisted commands can be executed. Destructive commands (migrate:fresh, db:wipe,
                    etc.) are blocked. All command executions are logged with user info and IP address.</p>
            </div>
        </div>
    </div>
</div>