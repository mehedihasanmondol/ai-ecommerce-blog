        ...
        <div class="p-4 border-b border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Name, email, mobile..."
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($appointments as $appointment)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $appointment->appointment_date->format('d M, Y') }}
                                </div>
                                ...
                                <div class="flex space-x-2">
                                    @if($appointment->status === 'pending')
                                        @if(auth()->user()->hasPermission('appointments.confirm'))
                                            <button wire:click="confirm({{ $appointment->id }})"
                                                class="text-blue-600 hover:text-blue-900" title="Confirm">
                                    ...
                                    @if($appointment->status === 'confirmed')
                                        @if(auth()->user()->hasPermission('appointments.complete'))
                                            <button wire:click="complete({{ $appointment->id }})"
                                                class="text-green-600 hover:text-green-900" title="Mark as Completed">
                                    ...
                                    @if(auth()->user()->hasPermission('appointments.edit-notes'))
                                        <button wire:click="openNotesModal({{ $appointment->id }})"
                                            class="text-purple-600 hover:text-purple-900" title="Add Notes">
                                    ...
                                    @if(auth()->user()->hasPermission('appointments.delete'))
                                        <button
                                            onclick="confirmDelete({{ $appointment->id }}, '{{ $appointment->customer_name }}')"
        ...
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button wire:click="saveNotes" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                        Save Notes
                    </button>
                </div>
            </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative rounded-lg shadow-xl max-w-md w-full p-6 border border-gray-200"
                ...
                <p class="text-sm text-gray-500 text-center mb-6">
                    Are you sure you want to delete the appointment for <span id="deleteCustomerName"
