<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        @if(auth()->user()->hasPermission('delivery-rates.create'))
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">৳{{ number_format($statistics['avg_base_rate'], 0) }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
        <div class="p-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1">
                    <div class="relative">
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search rates..."
                    ...
                    @forelse($rates as $rate)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-900">৳{{ number_format($rate->base_rate, 2) }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if(auth()->user()->hasPermission('delivery-rates.toggle-status'))
                                    <button wire:click="toggleStatus({{ $rate->id }})" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    @if(auth()->user()->hasPermission('delivery-rates.edit'))
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('delivery-rates.delete'))
                                        <button
                                            wire:click="$dispatch('confirm-delete', { id: {{ $rate->id }}, name: '{{ $rate->zone->name ?? 'Rate' }} - {{ $rate->method->name ?? '' }}' })"
