                    ...
                    <div class="bg-gray-50 px-6 py-6 flex-1 flex flex-col">
                        ...
                        @if($activeTab === 'library')
                            <div class="flex flex-col h-full">
                                ...
                                <div
                                    class="overflow-y-auto flex-1 mb-4 max-h-[calc(90vh-280px)]">
                                                ...
                                                @forelse($mediaLibrary as $media)
                                                    <div wire:click="toggleMediaSelection({{ $media->id }})"
                                                        ...
                                                        @if(in_array($media->id, $selectedMedia))
                                                            <div class="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1.5 z-20 shadow-lg">
                                                        ...
                                                        <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-200 flex flex-col justify-end p-3">
                                                            <div class="text-white text-xs space-y-1">
                                                                <p class="font-bold text-sm truncate">{{ $media->original_filename }}</p>
                                                                <div class="flex items-center justify-between text-[11px]">
                                                                    <span class="font-semibold">{{ $media->width }} × {{ $media->height }}</span>
                                        ...
                                        <div class="mb-4" x-data="{ uploading: false }">
                                            ...
                                            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                                @foreach($uploadedFiles as $index => $file)
                                                    <div
                                                        class="group bg-white border-2 border-gray-200 rounded-xl overflow-hidden hover:border-blue-400 transition-all duration-200 hover:shadow-lg">
                                                        {{-- Image --}}
                                                        <div class="aspect-square relative">
                                                            <img :src="croppedImages[{{ $index }}] || '{{ $file->temporaryUrl() }}'"
                                                                class="w-full h-full object-cover" x-data="{ imageIndex: {{ $index }} }">

                                                            {{-- Overlay with buttons (on hover) --}}
                                                            <div
                                                                class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end justify-center pb-3">
