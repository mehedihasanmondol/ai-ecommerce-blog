                    ...
                    <div class="bg-gray-50 px-6 py-6 flex-1 flex flex-col">
                        ...
                        @if($activeTab === 'library')
                            <div class="flex flex-col h-full">
                                ...
                                <div
                                    class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 overflow-y-auto flex-1 mb-4 max-h-[calc(90vh-280px)]">
                                    @forelse($mediaLibrary as $media)
                                        <div wire:click="toggleMediaSelection({{ $media->id }})"
                                            class="relative cursor-pointer group rounded-lg overflow-hidden border-2 h-fit
                                                                    {{ in_array($media->id, $selectedMedia) ? 'border-blue-500' : 'border-gray-200' }}">
                                            <img src="{{ $media->small_url }}" alt="{{ $media->alt_text }}"
                                                class="w-full h-32 object-cover">
                                            <div
                                                class="absolute inset-0 bg-black opacity-0 group-hover:opacity-50 transition-opacity flex items-center justify-center pointer-events-none">
                                                <div
                                                    class="opacity-0 group-hover:opacity-100 text-white text-xs text-center p-2 transition-opacity">
