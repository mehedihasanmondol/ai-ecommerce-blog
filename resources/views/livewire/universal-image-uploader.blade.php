<div>
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true"
            x-data="{ show: @entangle('showModal') }" x-show="show" x-cloak style="display: none;">
            ...
            <div class="flex min-h-screen items-center justify-center p-4">
                ...
                <div class="relative rounded-lg shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col border border-gray-200"
                    ...
                    {{-- Tab Contents --}}
                    <div class="bg-gray-50 px-6 py-6 flex-1 flex flex-col">
                        {{-- Library Tab --}}
                        @if($activeTab === 'library')
                            <div class="flex flex-col h-full">
                                ...
                                <div class="overflow-y-auto flex-1 mb-4 max-h-[calc(90vh-280px)]">
                                    <div class="columns-2 md:columns-4 lg:columns-5 gap-4 space-y-4">
                                        @forelse($mediaLibrary as $media)
                                            <div wire:click="toggleMediaSelection({{ $media->id }})"
                                                class="relative cursor-pointer group rounded-lg overflow-hidden border-2 transition-all break-inside-avoid mb-4
                                                                        {{ in_array($media->id, $selectedMedia) ? 'border-blue-500 shadow-lg ring-2 ring-blue-200' : 'border-gray-200 hover:border-blue-400 hover:shadow-md' }}">

                                                {{-- Selection Checkmark --}}
                                                @if(in_array($media->id, $selectedMedia))
                                                    <div
                                                        class="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1.5 z-20 shadow-lg">
                                                ...
                                                {{-- Info Overlay (on hover only) --}}
                                                <div
                                                    class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-200 flex flex-col justify-end p-3">
                                                    <div class="text-white text-xs space-y-1">
                                                        <p class="font-bold text-sm truncate">{{ $media->original_filename }}</p>
                                                        <div class="flex items-center justify-between text-[11px]">
                                                            <span class="font-semibold">{{ $media->width }} ×
                                                                {{ $media->height }}</span>
                                                            <span class="text-gray-300">{{ $media->formatted_size }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                {{-- Pagination & Actions (Fixed at bottom) --}}
                                <div class="border-t border-gray-200 pt-4 bg-gray-50">
                                    ...
                                    <div @dragover.prevent="dragging = true" @dragleave.prevent="dragging = false"
                                        ...
                                        <div class="mt-6">
                                            <label class="cursor-pointer">
                                                <span class="text-lg text-blue-600 hover:text-blue-700 font-semibold">Click to
                                                    upload</span>
                                        ...
                                        <div class="flex items-center justify-between mb-3 px-1">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500 mt-0.5">
                                                    Ready to upload •
                                                    {{ number_format(array_sum(array_map(fn($f) => $f->getSize() / 1024, $uploadedFiles)), 0) }}
                                                    KB total
                                                </p>
                                            </div>
                                            <button type="button" @click="uploading = true" wire:click="uploadImages"
                                                :disabled="uploading" :class="{ 'opacity-75 cursor-not-allowed': uploading }"
                                                ...
                                                <svg x-show="!uploading" class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12">
                                                    </path>
                                                </svg>
                                                <svg x-show="uploading" class="w-4 h-4 animate-spin" fill="none"
                                                    viewBox="0 0 24 24">
                                        ...
                                        <div class="overflow-y-auto max-h-[calc(90vh-280px)]">
                                            <div class="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
                                                @foreach($uploadedFiles as $index => $file)
                                                    <div
                                                        class="group bg-white border-2 border-gray-200 rounded-xl overflow-hidden hover:border-blue-400 transition-all duration-200 hover:shadow-lg break-inside-avoid mb-4">
                                                        {{-- Image with natural aspect ratio --}}
                                                        <div class="relative">
                                                            <img :src="croppedImages[{{ $index }}] || '{{ $file->temporaryUrl() }}'"
                                                                class="w-full h-auto object-cover" x-data="{ imageIndex: {{ $index }} }">

                                                            {{-- Overlay with buttons (on hover) --}}
                                                            <div
                                                                class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end justify-center pb-3">
