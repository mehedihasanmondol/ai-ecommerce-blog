<div class="flex items-center text-sm">
    @if($selectedZone || $selectedMethod)
        <button onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: { modalId: 'delivery-selector-modal' } }))"
            ...
            <div class="flex items-center space-x-1">
                @if($selectedZone)
                    <span class="font-medium text-gray-900 group-hover:text-green-700">{{ $selectedZone->name }}</span>
                @endif
                @if($selectedZone && $selectedMethod)
                    <span class="text-gray-400">•</span>
                @endif
                @if($selectedMethod)
                    <span class="text-gray-600 group-hover:text-green-600">{{ $selectedMethod->name }}</span>
    ...
    @else
        <button onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: { modalId: 'delivery-selector-modal' } }))"
                class="flex items-center text-gray-500 hover:text-green-600 hover:bg-gray-50 px-3 py-1.5 rounded-lg transition-colors">
