            ...
            <a href="#" class="group">
                <div class="bg-gray-50 rounded-lg p-6 text-center hover:shadow-lg transition">
                    @if($category->image_path)
                        <img src="{{ asset('storage/' . $category->image_path) }}" alt="{{ $category->name }}" class="w-20 h-20 mx-auto mb-3 object-cover rounded-full">
                    @else
                    ...
                    @endif
                    <h3 class="font-medium text-gray-900 group-hover:text-green-600 transition">{{ $category->name }}</h3>
                </div>
            </a>
            @endforeach
        </div>
