...
<div x-data="{ isSticky: false }" @scroll.window="isSticky = (window.pageYOffset > 100)" class="relative">
    ...
    <nav class="bg-red-600 text-white transition-all duration-300"
        :class="{ 'fixed top-0 left-0 right-0 z-50 shadow-lg': isSticky }"
        ...
        <div class="container mx-auto px-4">
            <div class="flex items-center">
                ...
                <div x-show="isSticky" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:leave-end="opacity-0 -translate-x-4" class="mr-4 flex-shrink-0" style="display: none;">
                    @if($siteLogo = \App\Models\SiteSetting::get('site_logo'))
                        <a href="{{ route('home') }}"
                            class="flex items-center bg-white px-3 py-2 -my-1 rounded-md shadow-sm hover:shadow-md transition">
                            <img src="{{ asset('storage/' . $siteLogo) }}"
                                alt="{{ \App\Models\SiteSetting::get('site_name', config('app.name')) }}"
                                class="h-10 object-contain">
                        </a>
                    @else
                        <a href="{{ route('home') }}" class="text-xl font-bold text-white hover:text-gray-100 transition">
                            {{ \App\Models\SiteSetting::get('site_name', config('app.name')) }}
                        </a>
                    @endif
                </div>
