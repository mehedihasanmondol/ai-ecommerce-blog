...
<body class="font-sans antialiased bg-gray-100" x-data="{ sidebarOpen: true, mobileMenuOpen: false }">
    ...
    <div class="min-h-screen">
                ...
                <div class="flex justify-between h-16">
                    ...
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.dashboard') }}" class="flex items-center">
                            ...
                            @if($adminLogo)
                                <img src="{{ asset('storage/' . $adminLogo) }}" alt="{{ $siteName }} Admin"
                                    class="h-10 w-auto">
                            @else
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        @if($maintenanceMode)
                            <div
                                class="hidden md:flex items-center bg-orange-100 text-orange-800 px-3 py-1.5 rounded-lg text-xs font-medium border border-orange-200">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open"
                                ...
                                @if(auth()->user()->media)
                                    <img src="{{ auth()->user()->media->small_url }}" alt="{{ auth()->user()->name }}"
                ...
                @if(auth()->user()->hasPermission('feedback.view'))
                    ...
                    <a href="{{ route('admin.feedback.index') }}"
                        ...
                        @if($pendingFeedbackCount > 0)
                            <span
                                class="ml-auto bg-orange-500 text-white text-xs px-2 py-1 rounded-full">{{ $pendingFeedbackCount }}</span>
                        @endif
                    ...
                    @if(auth()->user()->hasPermission('appointments.view'))
                        <a href="{{ route('admin.appointments.index') }}"
                            ...
                            @if($pendingAppointments > 0)
                                <span
                                    class="ml-auto bg-orange-500 text-white text-xs px-2 py-1 rounded-full">{{ $pendingAppointments }}</span>
                            @endif
                        </a>
                    @endif
                ...
                @if(auth()->user()->hasPermission('contact-messages.view'))
                    ...
                    <a href="{{ route('admin.contact.messages.index') }}"
                        ...
                        @if($unreadMessagesCount > 0)
                            <span
                                class="ml-auto bg-blue-500 text-white text-xs px-2 py-1 rounded-full">{{ $unreadMessagesCount }}</span>
                        @endif
        ...
        <main :class="sidebarOpen ? 'lg:ml-64' : 'lg:ml-0'"
            class="pt-16 transition-all duration-300 ease-in-out min-h-screen">
            <div class="py-6">
                <!-- Flash Messages -->
                @if(session('success'))
